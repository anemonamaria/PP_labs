...
permutari4([P1, P2, P3, P4], Permutare) :-	length(Permutare, 4),
										member(P1, Permutare),
										member(P2, Permutare),
										member(P3, Permutare),
										member(P4, Permutare).
...
alternare(_, _, 0, []).
alternare(A1, A2, Len, [A1 | Res]) :- L is Len - 1, Len > 0,
    alternare(A2, A1, L, Res).
...
factorial2(1, 1).
factorial2(Nr, Fact) :- factorial2(Nr1, Fact1), Nr is Nr1 + 1, Fact is Fact1 * Nr.
...
auCopil(_, _) :- false.
auCopil(P1, P2) :- parinte(Copil, P1), parinte(Copil, P2), P1 \== P2.

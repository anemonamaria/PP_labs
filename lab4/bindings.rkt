...
(define (compute-length get-line-segment get-start-point get-end-point)
    (let* ((line-segment (get-line-segment))
         (start (get-start-point line-segment)) (end (get-end-point line-segment)))
...
(define (distance x y)
  (compute-length (lambda null (cons x y)) car cdr))
...
(define (compute-perimeter points)
  (cond
    ((null? points) 0)
    (else (+ (distance (car points) (cond
                                      ((null? (cdr points)) null)
                                      (else (car (cdr points))))) (compute-perimeter (cdr points))))))

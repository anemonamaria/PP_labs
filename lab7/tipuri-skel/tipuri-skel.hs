...
dotV (V vx vy vz) (V vx' vy' vz') = vx * vx' + vy * vy' + vz * vz'
...
insertElem root@(Nod a left right) x
  | a > x = Nod a (insertElem left x) right
  | a < x = Nod a left (insertElem right x)
  | a == x = root

findElem :: (Ord a, Eq a) => BST a -> a -> Maybe a
findElem BSTNil _ = Nothing
findElem (Nod a left right) x
  | a < x = findElem right x
  | a > x = findElem left x
  | a == x = Just a
...
mapTree f (Nod a left right) = Nod (f a) (mapTree f left) $ mapTree f right

foldlTree :: (b -> a -> b) -> b -> BST a -> b
foldlTree _ acc BSTNil = acc
foldlTree f acc (Nod a left right) = foldlTree f newAcc right
  where newAcc = foldlTree f (f acc a) left

...
arme_marca(Marca, ListaArme) :- conduce(_, Marca), findall(Y, suspect(_:Marca:Y), ListaArme).
...
marci_arma_unica(ListaMarci) :- findall(Marca, arme_marca(Marca, [_]), ListaMarci).
check6:- tests([

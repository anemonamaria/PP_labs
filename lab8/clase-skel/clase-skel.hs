...
class (Ord a) => PQueue pq a where
    ...
    toList  que
      | isEmpty que = []
      | otherwise = x : (toList (pop que))
        where Just x = top que
...
instance (Ord a) => PQueue ListPQ a where
    ...
    insert elem (LPQ []) = LPQ [elem]
    insert elem (LPQ (x : xs))
      | fst elem >= fst x = LPQ (elem : x : xs)
      | otherwise = insert x (insert elem (LPQ xs))
...
merge (Empty rank1) (Empty rank2) = (Empty rank1)
merge (Empty rank1) (Node rank2 root2 left2 right2) = (Node rank2 root2 left2 right2)
merge (Node rank2 root2 left2 right2) (Empty rank1) = (Node rank2 root2 left2 right2)
merge (Node rank1 root1 left1 right1) (Node rank2 root2 left2 right2)
  | fst root1 <  fst root2 = merge (Node rank2 root2 left2 right2) (Node rank1 root1 left1 right1)
  | rank1 < rank2 = (Node (rank2 + 1) root1 left1 (merge right1 (Node rank2 root2 left2 right2)))
  | otherwise = (Node (rank1 + 1) root1 (merge right1 (Node rank2 root2 left2 right2)) left1)

inorder :: LeftistPQ a -> [(Prio, a)]
inorder (Empty _) = []
inorder (Node _ root2 left2 right2) = inorder(left2) ++ [root2] ++ inorder(right2)
